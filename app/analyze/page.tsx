...
import { getVideoInfo, analyzeStatic, getLiveChatId, analyzeLive, connectTwitchChannel, disconnectTwitchChannel, getTwitchMessages, getTwitchAnalytics, getTwitchSuggestions, connectKickChannel, disconnectKickChannel, getKickMessages, getKickAnalytics, getKickSuggestions } from "@/app/services/api";
...
export default function AnalyzePage() {
    ...
    }, [activeLiveChatId, nextPageToken]);
            ...
            {stats && (
                <section className="px-4 pb-16">
                    <div className="w-full max-w-5xl mx-auto space-y-6">
                        ...
                        {/* Live Stream Suggestions (Twitch or Kick) */}
                        {isLiveMode && liveSuggestions && liveSuggestions.suggestions && (
                            <div className="bg-[#1a1a1a] rounded-xl p-6">
                                ...
                                <div className="space-y-3">
                                    {liveSuggestions.suggestions.map((s: string, i: number) => (
                                        <div key={i} className="bg-[#222] rounded-lg p-4">
                                            <p className="text-gray-300 text-sm">{s}</p>
                                        </div>
                                    ))}
                                </div>
                                {liveSuggestions.note && (
                                    <p className="text-gray-500 text-xs mt-3">{liveSuggestions.note}</p>
                                )}
                            </div>
                        )}
